...
public void SQL_CreateTable_Callback(Database db, DBResultSet results, const char[] error, any data)
{
	...
	RunLongFastQuery(bSuccess, "CREATE GetWeightedPoints",
		...
		"DECLARE cur CURSOR FOR SELECT points FROM %splayertimes WHERE (auth = steamid OR partner = steamid) AND points > 0.0 ORDER BY points DESC; " ...
...
public Action Command_Tier(int client, int args)
{
	...
	Shavit_PrintToChat(client, "%T", "CurrentTier", client, gS_ChatStrings.sOrange, sMap, gS_ChatStrings.sText, gS_ChatStrings.sOrange, tier, gS_ChatStrings.sText);
...
public Action Command_Rank(int client, int args)
{
	...
	{
		Shavit_PrintToChat(client, "%T", "Unranked", client, gS_ChatStrings.sOrange, target, gS_ChatStrings.sText);

		return Plugin_Handled;
	}

	Shavit_PrintToChat(client, "%T", "Rank", client, gS_ChatStrings.sOrange, target, gS_ChatStrings.sText, gS_ChatStrings.sOrange, (gA_Rankings[target].iRank > gI_RankedPlayers)? gI_RankedPlayers:gA_Rankings[target].iRank, gS_ChatStrings.sText, gS_ChatStrings.sOrange, gI_RankedPlayers, gS_ChatStrings.sText, gS_ChatStrings.sOrange, gA_Rankings[target].fPoints, gS_ChatStrings.sText);
...
public Action Command_SetTier(int client, int args)
{
	...
	Shavit_PrintToChat(client, "%T", "SetTier", client, gS_ChatStrings.sOrange, tier, gS_ChatStrings.sText);
...
public Action Command_RecalcMap(int client, int args)
{
	...
	if(client >= 1)
	{
		Shavit_PrintToChat(client, "Done.");
	}
...
void UpdateAllPoints(bool recalcall = false)
{
	...
	{
		FormatEx(sQuery, 256, "UPDATE %susers SET points = GetWeightedPoints(auth) WHERE auth IN (SELECT DISTINCT LEAST(auth, partner) FROM playertimes UNION SELECT DISTINCT GREATEST(auth, partner) FROM playertimes);",
			gS_MySQLPrefix, gS_MySQLPrefix);
	}

	else
	{
		FormatEx(sQuery, 256, "UPDATE %susers SET points = GetWeightedPoints(auth) WHERE lastlogin > %d AND auth IN (SELECT DISTINCT LEAST(auth, partner) FROM playertimes UNION SELECT DISTINCT GREATEST(auth, partner) FROM playertimes);",
