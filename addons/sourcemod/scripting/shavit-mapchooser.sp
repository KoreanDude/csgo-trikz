...
#define VoteEndSound "yatta/endvote.mp3"
#define VoteMapSound "yatta/startyourvoting.mp3"
#define ChangeMapSound "yatta/kl_initializing.mp3"
...
public void OnPluginStart()
{
	...
	g_cvMapVoteDuration = CreateConVar("smc_mapvote_duration", "30", "Duration of time in seconds that map vote menu should be displayed for", _, true, 0.1, false);
...
public void OnMapStart()
{
	...
	PrecacheSound(VoteEndSound);
	PrecacheSound(VoteMapSound);
	PrecacheSound(ChangeMapSound);
}
...
public Action Timer_OnMapTimeLeftChanged(Handle Timer)
{
	...
	if(g_bDebug)
	{
		DebugPrint("{green}[SMC]{lightgreen} OnMapTimeLeftChanged: maplist_length=%i mapvote_started=%s mapvotefinished=%s", g_aMapList.Length, g_bMapVoteStarted ? "true" : "false", g_bMapVoteFinished ? "true" : "false");
	}
	#endif
	...
	if(GetMapTimeLeft(timeleft))
	{
		if(!g_bMapVoteStarted && !g_bMapVoteFinished)
		{
			...
			switch(mapvoteTime)
			{
				case (10 * 60) - 3:
				{
					CPrintToChatAll("{green}[SMC]{lightgreen} 10 minutes until map vote.");
				}
				case (5 * 60) - 3:
				{
					CPrintToChatAll("{green}[SMC]{lightgreen} 5 minutes until map vote.");
				}
				case 60 - 3:
				{
					CPrintToChatAll("{green}[SMC]{lightgreen} 1 minute until map vote.");
				}
				case 30 - 3:
				{
					CPrintToChatAll("{green}[SMC]{lightgreen} 30 seconds until map vote.");
				}
				case 5 - 3:
				{
					CPrintToChatAll("{green}[SMC]{lightgreen} 5 seconds until map vote.");
				}
			}
		}
	}
...
void CheckTimeLeft()
{
	...
	if(GetMapTimeLeft(timeleft) && timeleft > 0)
	{
		...
		if(g_bDebug)
		{
			DebugPrint("{green}[SMC]{lightgreen} CheckTimeLeft: timeleft=%i startTime=%i", timeleft, startTime);
		}
		#endif
		...
		if(timeleft - startTime <= 0)
		{
			...
			if(g_bDebug)
			{
				DebugPrint("{green}[SMC]{lightgreen} CheckTimeLeft: Initiating map vote ...", timeleft, startTime);
			}
			#endif
	...
	else
	{
		if(g_bDebug)
		{
			DebugPrint("{green}[SMC]{lightgreen} CheckTimeLeft: GetMapTimeLeft=%s timeleft=%i", GetMapTimeLeft(timeleft) ? "true" : "false", timeleft);
		}
	}
	#endif
}
...
void InitiateMapVote(MapChange when)
{
	...
	menu.DisplayVoteToAll(RoundFloat(g_cvMapVoteDuration.FloatValue));
	
	CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Nextmap Voting Started");
	
	EmitSoundToAll(VoteMapSound, SOUND_FROM_PLAYER, SNDCHAN_STATIC);
}
...
public void Handler_MapVoteFinished(Menu menu, int num_votes, int num_clients, const int[][] client_info, int num_items, const int[][] item_info)
{
	if (g_cvMapVoteRunOff.BoolValue && num_items > 1)
	{
		...
		if (winningvotes < required)
		{
			...
			g_hVoteMenu.DisplayVoteToAll(RoundFloat(g_cvMapVoteDuration.FloatValue));
			...
			CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Starting Runoff", g_cvMapVoteRunOffPerc.FloatValue, info1, map1percent, info2, map2percent);
...
public void Handler_VoteFinishedGeneric(Menu menu, int num_votes, int num_clients, const int[][] client_info, int num_items, const int[][] item_info)
{
	...
	CPrintToChatAll("{green}[SMC]{lightgreen} #1 vote was %s (%s).", map, (g_ChangeTime == MapChange_Instant) ? "instant" : "map end");
	...
	if(StrEqual(map, "extend"))
	{
		...
		CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
		LogAction(-1, -1, "Voting for next map has finished. The current map has been extended.");
		EmitSoundToAll(VoteEndSound, SOUND_FROM_PLAYER, SNDCHAN_STATIC);
	...
	{
		CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		else if(g_ChangeTime == MapChange_Instant)
		{
			...
			EmitSoundToAll(ChangeMapSound, SOUND_FROM_PLAYER, SNDCHAN_STATIC);
		}
		
		g_bMapVoteStarted = false;
		g_bMapVoteFinished = true;
		
		CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Nextmap Voting Finished", displayName, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
		...
		case MenuAction_VoteCancel:
		{
			...
			if(param1 == VoteCancel_NoVotes)
			{
				...
				if(strcmp(map, "extend", false) != 0 && strcmp(map, "dontchange", false) != 0)
				{
					...
					SetNextMap(map);
					CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Nextmap Voting Finished", map, 0, 0);
					LogAction(-1, -1, "Voting for next map has finished. Nextmap: %s.", map);
					g_bMapVoteFinished = true;
				}
			}
...
void ExtendMap(int time = 0)
{
	...
	CPrintToChatAll("{green}[SMC]{lightgreen} The map was extended for %.1f minutes.", time / 60.0);
...
public void LoadZonedMapsCallback(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("{green}[SMC]{lightgreen} - (LoadMapZonesCallback) - %s", error);
		return;	
	}
...
public void LoadZonedMapsCallbackMixed(Database db, DBResultSet results, const char[] error, any data)
{
	if(results == null)
	{
		LogError("{green}[SMC]{lightgreen} - (LoadMapZonesCallbackMixed) - %s", error);
		return;	
	}
...
void SMC_NominateMatches(int client, const char[] mapname)
{
	...
	switch (subNominateMenu.ItemCount) 
	{
    	case 0:
    	{
    		if (isCurrentMap) 
    		{
				CPrintToChat(client, "{green}[SMC]{lightgreen} %t", "Can't Nominate Current Map");
			}
			else if (isOldMap) 
			{
				CPrintToChat(client, "{green}[SMC]{lightgreen} %s %t", oldMapName, "Recently Played");
			}
			else 
			{
				CPrintToChat(client, "{green}[SMC]{lightgreen} %t", "Map was not found", mapname);	
			}
...
public Action Command_ForceMapVote(int client, int args)
{
	if(g_bMapVoteStarted || g_bMapVoteFinished)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} Map vote already %s", (g_bMapVoteStarted) ? "initiated" : "finished");
	}
	else
	{
...
public Action Command_Nominate(int client, int args)
{
	...
	if (g_cvNominateMatches.BoolValue)
	{
		...
		if(SMC_FindMap(mapname, mapname, sizeof(mapname)))
		{
			...
			if(idx != -1)
			{
				CPrintToChat(client, "{green}[SMC]{lightgreen} %s %t", mapname, "Recently Played");
				return Plugin_Handled;
			}
...
public Action Command_UnNominate(int client, int args)
{
	...
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} You haven't nominated a map");
		return Plugin_Handled;
	}
	...
	if(idx != -1)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} Successfully removed nomination for '%s'", g_cNominatedMap[client]);
...
void Nominate(int client, const char mapname[PLATFORM_MAX_PATH])
{
	...
	if(idx != -1)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} %t", "Map Already Nominated");
		return;
	}
	...
	CPrintToChatAll("{green}[SMC]{lightgreen} %t", "Map Nominated", name, mapname);
}
...
public Action Command_RockTheVote(int client, int args)
{
	if(!IsRTVEnabled())
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} %t", "RTV Not Allowed");
	}
	else if(g_bMapVoteStarted)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} %t", "RTV Started");
	}
	else if(g_bRockTheVote[client])
	{
		int needed = GetRTVVotesNeeded();
		CPrintToChat(client, "{green}[SMC]{lightgreen} You have already RTVed, if you want to un-RTV use the command sm_unrtv (%i more %s needed)", needed, (needed == 1) ? "vote" : "votes");
	}
	else if(g_cvRTVMinimumPoints.IntValue != -1 && Shavit_GetPoints(client) <= g_cvRTVMinimumPoints.FloatValue)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} You must be a higher rank to RTV!");
	}
	else if(GetClientTeam(client) == CS_TEAM_SPECTATOR && !g_cvRTVAllowSpectators.BoolValue)
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} Spectators have been blocked from RTVing");
	}
	else
	{
		RTVClient(client);
...
void CheckRTV(int client = 0)
{
	...
	if(needed > 0)
	{
		if(client != 0)
		{
			CPrintToChatAll("{green}[SMC]{lightgreen} %t", "RTV Requested", name, rtvcount, total);
		}
	}
	else
	{
		if(g_bMapVoteFinished)
		{
			...
			if(client != 0)
			{
				CPrintToChatAll("{green}[SMC]{lightgreen} %N wants to rock the vote! Map will now change to %s...", client, map);
			}
			else
			{
				CPrintToChatAll("{green}[SMC]{lightgreen} RTV vote now majority, map changing to %s...", map);
			}
		...
		else
		{
			if(client != 0)
			{
				CPrintToChatAll("{green}[SMC]{lightgreen} %N wants to rock the vote! Map vote will now start...", client);
			}
			else
			{
				CPrintToChatAll("{green}[SMC]{lightgreen} RTV vote now majority, map vote starting...");
			}
...
public Action Command_UnRockTheVote(int client, int args)
{
	if(!IsRTVEnabled())
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} RTV has not been enabled yet");
	}
	...
	else if(g_bRockTheVote[client])
	{
		...
		if(needed > 0)
		{
			CPrintToChatAll("{green}[SMC]{lightgreen} %N no longer wants to rock the vote! (%i more votes needed.)", client, needed);
		}
	}
...
public Action Command_NomList(int client, int args)
{
	...
	{
		CPrintToChat(client, "{green}[SMC]{lightgreen} No Maps Nominated");
		return Plugin_Handled;
	}
...
public Action Command_Debug(int client, int args)
{
	...
	{
		g_bDebug = !g_bDebug;
		CPrintToChat(client, "{green}[SMC]{lightgreen} Debug mode: %s", g_bDebug ? "ENABLED" : "DISABLED");
