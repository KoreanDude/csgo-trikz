...
{
	name = "Nade Tails",
	author = "InternetBully & ^kS",
	version = "3.0",
	description = "Adds tails to projectiles",
	url = "https://github.com/ksgoescoding/NadeTails"
};
...
public OnPluginStart()
{
	...
	//CVARs
	g_Enabled 				= CreateConVar("sm_tails_enabled", "1", "Enables Nade Tails (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_AllowPlayers 		= CreateConVar("sm_tails_allowplayers", "1", "Allow players to use nade tails with !tails (0/1)", FCVAR_NONE, true, 0.0, true, 1.0);
	g_DefaultAlpha		= CreateConVar("sm_tails_defaultalpha", "255", "Default alpha for trails (0 is invisible, 255 is solid).", FCVAR_NONE, true, 0.0, true, 255.0);
	g_DefaultOn			= CreateConVar("sm_tails_defaulton", "1", "Tails on for all users, Set to 0 to require user to type !tails to use", FCVAR_NONE, true, 0.0, true, 1.0);
	
	//Projectiles to put tails on
	g_EnableHETails		= CreateConVar("sm_tails_hegrenade", "1", "Enables Nade Tails on HE Grenades (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_EnableFlashTails	= CreateConVar("sm_tails_flashbang", "1", "Enables Nade Tails on Flashbangs (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_EnableSmokeTails	= CreateConVar("sm_tails_smoke", "1", "Enables Nade Tails on Smoke Grenades (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_EnableDecoyTails	= CreateConVar("sm_tails_decoy", "1", "Enables Nade Tails on Decoy Grenades (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_EnableMolotovTails	= CreateConVar("sm_tails_molotov", "1", "Enables Nade Tails on Molotovs (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	g_EnableIncTails		= CreateConVar("sm_tails_incendiary", "1", "Enables Nade Tails on Incendiary Grenades (0/1).", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	//size and time
	g_TailTime 			= CreateConVar("sm_tails_tailtime", "1.0", "Time the tail stays visible.", FCVAR_NONE, true, 0.0, true, 25.0);
	g_TailFadeTime		= CreateConVar("sm_tails_tailfadetime", "1", "Time for tail to fade over.", FCVAR_NONE);
	g_TailWidth			= CreateConVar("sm_tails_tailwidth", "1.0", "Width of the tail.", FCVAR_NONE);
...
public mh_TailMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if(target == 0)
				PrintToChat(param1, "[NadeTails] %s", "Player no longer available");
			else if(!CanUserTarget(param1, target))
				PrintToChat(param1, "[NadeTails] %s", "Unable to target");
			else
			{
