...
Handle gH_Forwards_EnterZonePartnerMode = null;
...
public void OnPluginStart()
{
	...
	gH_Forwards_EnterZonePartnerMode = CreateGlobalForward("Shavit_OnEnterZonePartnerMode", ET_Event, Param_Cell, Param_Cell, Param_Cell, Param_Cell, Param_Cell, Param_Cell);
...
public void Shavit_OnRestart(int client, int track)
{
	if(gCV_TeleportToStart.BoolValue)
	{
		...
		else if((iIndex = GetZoneIndex(Zone_Start, track)) != -1)
		{
			...
			if(StrEqual(gS_Map, "trikz_randomraider_fix") && track == Track_Solobonus)
			{
				fCenter[2] += 8.0;
			}
...
public void Shavit_OnEnd(int client, int track)
{
	if(gCV_TeleportToEnd.BoolValue)
	{
		...
		if((iIndex = GetZoneIndex(Zone_End, track)) != -1)
		{
			...
			if(StrEqual(gS_Map, "trikz_research_laboratories") && track == Track_Main)
			{
				fCenter[1] += 256.0;
			}
			
			if(StrEqual(gS_Map, "trikz_eonia") && track == Track_Bonus)
			{
				fCenter[0] += 256.0;
			}
...
public void StartTouchPost(int entity, int other)
{
	...
	int track = gA_ZoneCache[entityzone].iZoneTrack; //Get zone track
	int iPartner = Trikz_FindPartner(other);
	
	gB_InsideZone[other][gA_ZoneCache[gI_EntityZone[entity]].iZoneType][track] = true;
	...
	switch(gA_ZoneCache[gI_EntityZone[entity]].iZoneType)
	{
		...
		case Zone_Stop:
		{
			if(status == Timer_Running)
			{
				...
				if((iPartner != -1) && (Shavit_GetClientTrack(other) != Track_Solobonus))
				{
					Shavit_PrintToChat(iPartner, "%T", "ZoneStopEnter", iPartner, gS_ChatStrings.sWarning, gS_ChatStrings.sVariable2, gS_ChatStrings.sWarning);
				}
			}
		}
		...
		case Zone_Start:
		{
			if(iPartner != -1)
			{
				if(gB_InsideZone[other][Zone_Start][track] && gB_InsideZone[iPartner][Zone_Start][track] && Shavit_GetClientTrack(other) != Track_Solobonus)
				{
					Shavit_StopTimer(other);
					
					Call_StartForward(gH_Forwards_EnterZonePartnerMode);
					Call_PushCell(other);
			...
			if(gB_InsideZone[other][Zone_Start][Track_Solobonus])
			{
				Shavit_StopTimer(other);
			}
		}
		...
		case Zone_End:
		{
			if(status == Timer_Running)
			{
				if(iPartner != -1)
				{ 
					//if(gB_InsideZone[other][Zone_End][Track_Main] && gB_InsideZone[iPartner][Zone_End][Track_Main] && Shavit_GetClientTrack(other) == Track_Main)
					if(gB_InsideZone[other][Zone_End][Track_Main] && Shavit_GetClientTrack(other) == Track_Main
					|| gB_InsideZone[iPartner][Zone_End][Track_Main] && Shavit_GetClientTrack(iPartner) == Track_Main)
					{
						Shavit_FinishMap(other, track);
					}
					
					if(gB_InsideZone[other][Zone_End][Track_Bonus] && Shavit_GetClientTrack(other) == Track_Bonus
					|| gB_InsideZone[iPartner][Zone_End][Track_Bonus] && Shavit_GetClientTrack(iPartner) == Track_Bonus)
					{
						Shavit_FinishMap(other, track);
					}
				}
				
				if(gB_InsideZone[other][Zone_End][Track_Solobonus] && Shavit_GetClientTrack(other) == Track_Solobonus)
				{
					Shavit_FinishMap(other, track);
				}
			}
...
public void EndTouchPost(int entity, int other)
{
	...
	if(iPartner != -1)
	{
		if(type == Zone_Start && (track == Track_Main || track == Track_Bonus))
		{
			int iCStyle = Shavit_GetBhopStyle(other);
			int iPStyle = Shavit_GetBhopStyle(iPartner);
			
			if(gB_InsideZone[iPartner][type][track] && iCStyle == iPStyle)
			{
				Shavit_UpdateLocalCache(other);
				
				Shavit_StartTimer(other, track);
				Shavit_StartTimer(iPartner, track);
			}
		}
	}
	...
	{
		Shavit_UpdateLocalCacheSB(other);
...
public void TouchPost(int entity, int other)
{
	...
	if(gB_InsideZone[other][Zone_Start][Track_Solobonus])
	{
		...
		{
			Shavit_SetClientTrack(other, Track_Solobonus);
		}
	}
}
